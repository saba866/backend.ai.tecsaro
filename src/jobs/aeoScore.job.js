...
// //
// // 1. PRESENCE RATE    (35pts) — Are you appearing at all?
// // 2. WIN RATE         (25pts) — When you appear, do you win?
// // 3. QUERY COVERAGE   (20pts) — What % of queries do you appear in?
// // 4. COMPETITIVE POS  (15pts) — How do you rank vs competitors?
// // 5. TECHNICAL        (5pts)  — Schema, crawl hygiene
// // ─────────────────────────────────────────
...
//   // ── GROUP MENTIONS BY ANSWER → WIN/LOSS/SHARED/MISSED ──
//   const grouped = {};
//   for (const m of mentionData || []) {
//     if (!grouped[m.answer_id]) grouped[m.answer_id] = [];
//     grouped[m.answer_id].push(m);
//   }
...
//     if (brandMentioned && competitorsFound.length === 0) wins++;
//     else if (!brandMentioned && competitorsFound.length > 0) losses++;
//     else if (brandMentioned && competitorsFound.length > 0) shared++;
//     else missed++;
...
//   const totalAnswers      = Object.keys(grouped).length;
...
//   console.log(`   Wins:    ${wins} | Losses: ${losses} | Shared: ${shared} | Missed: ${missed}`);

//   // ─────────────────────────────────────────
//   // SCORE CALCULATION
//   // ─────────────────────────────────────────

//   // ── 1. PRESENCE RATE (35 points) ──
//   // How often brand appears across all tracked queries
//   // Most important metric — weighted highest
//   const presenceScore = totalAnswers > 0
//     ? Math.round(brandPresenceRate * 35)
//     : 0;

//   // ── 2. WIN RATE (25 points) ──
//   // Wins = full credit, Shared = half credit, Loss/Missed = 0
//   // Shared counts because appearing with competitor is better than not appearing
//   const winScore = totalAnswers > 0
//     ? Math.round(((wins + shared * 0.5) / totalAnswers) * 25)
//     : 0;

//   // ── 3. QUERY COVERAGE (20 points) ──
//   // What % of queries does brand appear in at least once
//   // Counts both wins and shared — missing from query entirely = bad
//   const coveredQueries  = wins + shared;
//   const coverageRate    = totalAnswers > 0 ? coveredQueries / totalAnswers : 0;
//   const coverageScore   = Math.round(coverageRate * 20);

//   // ── 4. COMPETITIVE POSITION (15 points) ──
//   // Win rate specifically when competing against other brands
//   // Only counts queries where at least one brand appeared
//   const contestedAnswers  = wins + losses + shared;
//   const competitiveRate   = contestedAnswers > 0 ? wins / contestedAnswers : 0;
//   const competitiveScore  = Math.round(competitiveRate * 15);

//   // ── 5. TECHNICAL READINESS (5 points) ──
//   // Schema markup + crawl quality — hygiene factor
//   // Small weight because technical won't save a brand with no presence
//   let technicalScore = 0;
//   if (pageCount > 0 && understoodPages > 0) {
//     const schemaRatio  = pageCount > 0 ? Math.min(1, schemaCount / pageCount) : 0;
//     const crawlRatio   = pageCount > 0 ? Math.min(1, understoodPages / pageCount) : 0;
...
//     { category: "Presence Rate",       points: presenceScore,    max: 35, detail: `Brand in ${(brandPresenceRate * 100).toFixed(1)}% of AI answers` },
//     { category: "Win Rate",            points: winScore,         max: 25, detail: `${wins} wins, ${shared} shared out of ${totalAnswers} answers` },
//     { category: "Query Coverage",      points: coverageScore,    max: 20, detail: `Covered ${coveredQueries}/${totalAnswers} queries` },
//     { category: "Competitive Position",points: competitiveScore, max: 15, detail: `Won ${wins}/${contestedAnswers} contested queries` },
//     { category: "Technical Readiness", points: technicalScore,   max: 5,  detail: `${schemaCount} schemas, ${understoodPages}/${pageCount} pages understood` },
//   ];

//   // ── LOG SCORE BREAKDOWN ──
//   console.log("\n📊 ── SCORE BREAKDOWN ───────────────────");
//   for (const item of scoreComponents) {
//     const filled  = Math.round((item.points / item.max) * 10);
...
//   await checkAeoScoreAlert(planId, totalScore);
...
//   // ── LOAD ALL DATA IN PARALLEL (already fast) ──
//   const [
//     { count: pageCount },
//     { count: understoodPages },
//     { count: promptCount },
...
//   // ── SCORING ──
//   const presenceScore   = totalAnswers > 0 ? Math.round(brandPresenceRate * 35) : 0;
//   const winScore        = totalAnswers > 0 ? Math.round(((wins + shared * 0.5) / totalAnswers) * 25) : 0;
//   const coveredQueries  = wins + shared;
//   const coverageScore   = Math.round((totalAnswers > 0 ? coveredQueries / totalAnswers : 0) * 20);
//   const contestedAnswers  = wins + losses + shared;
//   const competitiveScore  = Math.round((contestedAnswers > 0 ? wins / contestedAnswers : 0) * 15);
...
//   const breakdown = {
//     pageCount: pageCount || 0, understoodPages: understoodPages || 0,
//     promptCount: promptCount || 0, aiAnswerCount: aiAnswerCount || 0,
//     competitorCount: competitorCount || 0, schemaCount: schemaCount || 0,
//     gapCount: gapCount || 0, recommendationCount: recommendationCount || 0,
//     brandPresenceRate: parseFloat(brandPresenceRate.toFixed(3)),
//     brandMentions, totalBrandChecks, competitorMentions,
//     totalAnswers, wins, losses, shared, missed, scoreComponents,
//   };

//   // ── SAVE — delete + insert in parallel ──
//   await supabase.from("aeo_scores").delete().eq("plan_id", planId);
...
//   if (insertErr) { console.error("❌ Failed to save score:", insertErr.message); return null; }

//   // Fire alert check without blocking
