...
// ─────────────────────────────────────────
const PROMPT_CONFIG = {
  free:    { generate: 20,  select_max: 10, select_min: 3  },
  starter: { generate: 50,  select_max: 20, select_min: 5  },
  pro:     { generate: 100, select_max: 50, select_min: 10 },
};

// ─────────────────────────────────────────
// GET EFFECTIVE TIER + CONFIG
// Source of truth: billing_profiles.current_plan_slug
// Fallback: plans.tier → PROMPT_CONFIG lookup
// ─────────────────────────────────────────
async function getEffectiveConfig(planId) {
  // 1. Get plan row for user_id + fallback tier
  const { data: plan } = await supabase
    .from("plans")
    .select("id, name, website_url, tier, user_id, prompt_select_max, prompt_select_min, prompts_approved, pipeline_status")
    .eq("id", planId)
    .single();

  if (!plan) return { plan: null, tier: "free", config: PROMPT_CONFIG.free };

  // 2. Read current plan slug from billing_profiles (source of truth)
  ...
  // 3. Pull limits live from pricing_plans table
  const { data: pricingPlan } = await supabase
    .from("pricing_plans")
    .select("prompts_generate, prompts_select_max, prompts_select_min")
    .eq("slug", effectiveSlug)
    .eq("is_active", true)
    .maybeSingle();

  // Build config — live from pricing_plans, fallback to PROMPT_CONFIG map
  const config = pricingPlan
    ? {
        generate:   pricingPlan.prompts_generate   ?? PROMPT_CONFIG[effectiveSlug]?.generate   ?? 20,
        select_max: pricingPlan.prompts_select_max  ?? PROMPT_CONFIG[effectiveSlug]?.select_max ?? 10,
        select_min: pricingPlan.prompts_select_min  ?? PROMPT_CONFIG[effectiveSlug]?.select_min ?? 3,
      }
    : (PROMPT_CONFIG[effectiveSlug] ?? PROMPT_CONFIG.free);

  return { plan, tier: effectiveSlug, config };
}
...
export function isValidPrompt(promptText, brandName = "") {
  ...
  if (wordCount < 5)  { console.log(`   ❌ Too short (${wordCount} words): "${promptText}"`); return false; }
  if (wordCount > 25) { console.log(`   ❌ Too long (${wordCount} words): "${promptText}"`);  return false; }

  if (brandName && text.includes(brandName.toLowerCase())) {
    console.log(`   ❌ Contains brand name: "${promptText}"`); return false;
  }

  if (text.match(/\.(com|ai|co|io|org|net)/)) {
    console.log(`   ❌ Contains domain: "${promptText}"`); return false;
  }
  ...
  if (headingPhrases.some((p) => text.includes(p))) {
    console.log(`   ❌ Heading phrase: "${promptText}"`); return false;
  }
  ...
  if (!intentSignals.some((s) => text.includes(s))) {
    console.log(`   ❌ No intent signal: "${promptText}"`); return false;
  }
...
export const startPromptDiscovery = async (planId) => {
  ...
  // ── STEP 2: LOAD PLAN + EFFECTIVE TIER FROM BILLING ──
  const { plan, tier, config } = await getEffectiveConfig(planId);
  ...
  // Sync correct limits back to plans table immediately
  await supabase
    .from("plans")
    .update({
      tier:                  tier,
      prompts_generate_count: config.generate,
      prompt_select_max:     config.select_max,
      prompt_select_min:     config.select_min,
  ...
  if (!Array.isArray(parsed) || !parsed.length) {
    console.error("❌ Could not parse prompts — aborting");
    return;
  }
...
export const approvePrompts = async (planId, selectedIds) => {
  ...
  // Use effective config from billing_profiles
  const { plan, tier, config } = await getEffectiveConfig(planId);

  if (!plan) return { error: "Plan not found" };

  const max = config.select_max;
  const min = config.select_min;
  ...
  if (selectedIds.length > availableSlots) {
    return {
      error: manualCount > 0
        ? `You have ${manualCount} manual prompt(s) already. You can activate up to ${availableSlots} more (total cap: ${max}).`
        : `You can only activate up to ${max} prompts on the ${tier} plan`,
    };
  }
...
export const addManualPrompt = async (planId, promptText, intent = "informational") => {
  ...
  if ((activeCount || 0) >= max) {
    return {
      error:         `You've reached your limit of ${max} active prompts on the ${tier} plan. Remove one to add a new prompt.`,
      limit_reached: true,
    };
  }
...
export const removePrompt = async (planId, promptId) => {
  ...
  const { plan, config } = await getEffectiveConfig(planId);
  const max = config.select_max;
...
export const getPromptsForReview = async (planId) => {
  ...
  const selectMax    = config.select_max;
  const selectMin    = config.select_min;
  const manualUsed   = manualCount || 0;
  const availableSlots = Math.max(0, selectMax - manualUsed);
...
// ─────────────────────────────────────────
// SUGGEST NEW PROMPTS (weekly refresh)
// ─────────────────────────────────────────
export const suggestNewPrompts = async (planId) => {
  console.log("💡 Generating prompt suggestions for plan:", planId);

  const { plan } = await getEffectiveConfig(planId);
  if (!plan) return;
...
// ─────────────────────────────────────────
export const approveSuggestedPrompt = async (planId, promptId) => {
  const { tier, config } = await getEffectiveConfig(planId);
  ...
  if ((activeCount || 0) >= max) {
    return {
      error:         `You've reached your limit of ${max} active prompts on the ${tier} plan. Remove one to add this suggestion.`,
      limit_reached: true,
    };
  }
