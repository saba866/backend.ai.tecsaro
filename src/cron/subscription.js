...
export async function checkSubscriptions() {
  ...
  try {
    ...
    const { data: activePlans, error: fetchErr } = await supabase
      .from("plans")
      .select("id, user_id, name, tier, subscription_status, current_period_end, razorpay_subscription_id")
      .eq("subscription_status", "active")
      .in("tier", ["starter", "pro"])
    ...
    if (!activePlans?.length) {
      console.log("ℹ️  [subscriptionCheck] No active paid plans to check")
      return { active: 0, expired: 0, halted: 0 }
    }
    ...
    for (const plan of activePlans) {
      if (plan.current_period_end && plan.current_period_end < now) {
        console.log(`   ⚠️  Period ended: "${plan.name}"`)
        await supabase.from("plans").update({ subscription_status: "expired", tier: "free", pipeline_status: "idle" }).eq("id", plan.id)
        await supabase.from("billing_profiles").update({ subscription_status: "expired", current_plan_slug: "free", updated_at: now }).eq("user_id", plan.user_id)
        expired++
        continue
      }
      ...
      if (plan.razorpay_subscription_id) {
        ...
        if (status === "expired") {
          ...
          await supabase.from("plans").update({ subscription_status: "payment_failed" }).eq("id", plan.id)
          await supabase.from("billing_profiles").update({ subscription_status: "payment_failed", updated_at: now }).eq("user_id", plan.user_id)
          halted++
        } else if (status === "active") {
          active++
        } else {
    ...
    console.log(`\n✅ [subscriptionCheck] Complete`)
    console.log(`   Active: ${active} | Expired: ${expired} | Halted: ${halted} | Unknown: ${unknown}`)
    return { active, expired, halted, unknown }
