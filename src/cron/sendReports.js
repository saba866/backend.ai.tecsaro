...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f172a; color: #e2e8f0; margin: 0; padding: 24px;">
  <div style="max-width: 600px; margin: 0 auto;">
    ...
    <div style="text-align: center; color: #475569; font-size: 12px; border-top: 1px solid #1e293b; padding-top: 20px;">
      <p style="margin: 0;">Tecsaro AI· <a href="${APP_URL}/dashboard/settings" style="color: #64748b;">Unsubscribe</a></p>
    </div>
  </div>
</body>
</html>`.trim()
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; background: #0f172a; color: #e2e8f0; margin: 0; padding: 24px;">
  <div style="max-width: 600px; margin: 0 auto;">
    ...
    <div style="text-align: center; color: #475569; font-size: 12px; border-top: 1px solid #1e293b; padding-top: 20px;">
      <p style="margin: 0;">Tecsaro · <a href="${APP_URL}/dashboard/settings" style="color: #64748b;">Unsubscribe</a></p>
    </div>
  </div>
</body>
</html>`.trim()
...
// ── Fetch plans + emails via two separate queries (no join needed) ──
async function getPlansWithReports(reportField) {
  const { data: plans, error } = await supabase
    .from("plans")
    .select(`id, name, user_id, tier, ${reportField}`)
    .eq(reportField, true)
    .in("tier", ["starter", "pro"])
  ...
  if (profileErr) {
    console.error(`❌ [sendReports] Failed to fetch profiles:`, profileErr.message)
  }
  ...
  // Merge email + name into each plan
  ...
  try {
    ...
    for (let i = 0; i < plans.length; i += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(async (plan) => {
        ...
        if (!email) {
          console.log(`   ⏭️  No email for "${plan.name}" — skipping`)
          failed++
          return
        }
