...
const DISCOUNT_PLAN_MAP = {
  30: {
    starter:        process.env.RAZORPAY_PLAN_STARTER_30OFF,
    starter_yearly: process.env.RAZORPAY_PLAN_STARTER_YEARLY_30OFF,
    pro:            process.env.RAZORPAY_PLAN_PRO_30OFF,
    pro_yearly:     process.env.RAZORPAY_PLAN_PRO_YEARLY_30OFF,
  },
  50: {
    starter:        process.env.RAZORPAY_PLAN_STARTER_50OFF,
    starter_yearly: process.env.RAZORPAY_PLAN_STARTER_YEARLY_50OFF,
    pro:            process.env.RAZORPAY_PLAN_PRO_50OFF,
    pro_yearly:     process.env.RAZORPAY_PLAN_PRO_YEARLY_50OFF,
  },
  // 100% off — user pays ₹1/month. Create these as ₹1 plans in Razorpay dashboard.
  100: {
    starter:        process.env.RAZORPAY_PLAN_STARTER_100OFF,
    starter_yearly: process.env.RAZORPAY_PLAN_STARTER_YEARLY_100OFF,
    pro:            process.env.RAZORPAY_PLAN_PRO_100OFF,
    pro_yearly:     process.env.RAZORPAY_PLAN_PRO_YEARLY_100OFF,
  },
}
...
export const validateCoupon = async (req, res) => {
  ...
  const isDiscount = coupon.discount_type === "percent"
  const isTrial    = coupon.discount_type === "trial_days" || coupon.discount_type === "fixed"
  const isFull     = isDiscount && coupon.discount_value === 100

  let message   = ""
  let trialDays = null

  if (isFull) {
    message = `🎉 100% off! Pay just ₹1 to activate your subscription.`
  } else if (isDiscount) {
    message = `🎉 ${coupon.discount_value}% discount applied on your subscription!`
  } else if (isTrial) {
    trialDays = coupon.discount_value ?? 30
...
// ─────────────────────────────────────────────────────────────────
// POST /coupons/create-discounted-subscription
// 30% / 50% / 100% discount — creates Razorpay subscription on discounted plan
// 100% off plans are priced at ₹1 in the Razorpay dashboard
// ─────────────────────────────────────────────────────────────────
export const createDiscountedSubscription = async (req, res) => {
  const userId = req.user?.id
  const email  = req.user?.email
...
// ─────────────────────────────────────────────────────────────────
// POST /coupons/verify-discounted-subscription
// 30% / 50% / 100% — verify payment and activate plan
// ─────────────────────────────────────────────────────────────────
export const verifyDiscountedSubscription = async (req, res) => {
  const userId = req.user?.id
  const {
  ...
  try {
    ...
    const msg = coupon.discount_value === 100
      ? `🎉 ${planKey} plan activated at ₹1!`
      : `🎉 ${planKey} plan activated at ${coupon.discount_value}% off!`

    console.log(`✅ Discounted subscription verified: user=${userId} plan=${planKey} discount=${coupon.discount_value}%`)

    return apiResponse(res, 200, "Plan activated!", { plan: planKey, message: msg })
  } catch (err) {
    console.error("[verifyDiscountedSubscription]", err)
    return apiResponse(res, 500, "Failed to activate plan")
  }
