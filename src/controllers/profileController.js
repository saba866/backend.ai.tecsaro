...
export const syncProfile = async (req, res) => {
  ...
  console.log(`[syncProfile] user=${userId} name="${name}"`)
  ...
  try {
    ...
    if (!existing) {
      // First time — set free tier
      Object.assign(upsertData, {
        tier: "free",
      })
    } else {
      // Re-sync — don't touch tier
      Object.assign(upsertData, {
        tier: existing.tier,
      })
    }
    ...
    console.log(`✅ [syncProfile] synced: user=${userId} tier=${data.tier}`)
    return res.status(200).json({ success: true, data })
  ...
  try {
    ...
    if (!data) {
      ...
      if (createErr) {
        return res.status(200).json({
          success: true,
          data: { id: userId, name: name || null, email: email || null, avatar, tier: "free" },
        })
      }
      data = created
    }
